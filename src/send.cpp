...
void setup() {
    Serial.begin(115200);
    mySerial.begin(115200, SERIAL_8N1, 17, 16); // TX = GPIO 16, RX = GPIO 17
...
void loop() {
    ...
    temp = temp | 0x80;
    uint8_t gasVal = digitalRead(ANALOG_PIN);
    uint8_t arr[2] = {
      static_cast<uint8_t>(temp),
      static_cast<uint8_t>(gasVal),
      // static_cast<uint8_t>((analogValue >> 8) & 0xFF)
    };

    Serial.println(temp & 0x3F);
